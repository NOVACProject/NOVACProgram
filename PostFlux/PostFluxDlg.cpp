...
CPostFluxDlg::CPostFluxDlg(CWnd* pParent /*=nullptr*/)
...
CPostFluxDlg::~CPostFluxDlg()
{
    ...
    if (m_windFieldFromFile != nullptr)
    {
        delete m_windFieldFromFile;
        m_windFieldFromFile = nullptr;
    }
...
void CPostFluxDlg::DrawScan()
{
    ...
    else
    {
        ...
        if (m_show.columnError)
        {
            m_scanGraph.XYPlot(times, column, nullptr, nullptr, columnError, numSpec, Graph::CGraphCtrl::PLOT_FIXED_AXIS | Graph::CGraphCtrl::PLOT_CONNECTED);
        }
        else
        {
...
void	CPostFluxDlg::RetrieveWindField()
{
    ...
    else
    {
        ...
        if (SUCCESS != m_windFieldFromFile->InterpolateWindField(dt, m_calculator->m_wind))
        {
        ...
        if (m_windFieldFromFile->m_containsWindSpeed)
        {
        ...
        if (m_windFieldFromFile->m_containsWindDirection)
        {
        ...
        if (m_windFieldFromFile->m_containsPlumeHeight)
        {
...
void CPostFluxDlg::OnSaveEvalLog()
{
    ...
    if (m_calculator == nullptr || strlen(m_calculator->m_evaluationLog) < 3)
    {
...
void CPostFluxDlg::OnImportWindField()
{
    ...
    m_windFieldFromFile = new CWindFieldDatabase();

    FileHandler::CWindFileReader windFileReader;

    // Set the path to the file
    windFileReader.m_windFile = windFile;

    // Read the wind-file
    if (SUCCESS != windFileReader.ReadWindFile(*m_windFieldFromFile))
    {
    ...
    {
        message.Format("Successfully read %ld data points from file", m_windFieldFromFile->GetRecordNum());
        MessageBox(message);
    }
