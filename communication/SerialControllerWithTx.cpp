...
CSerialControllerWithTx::CSerialControllerWithTx(void)
{
    ...
    m_sleepFlag = false;
    m_radioID.Format("");
    SetupElectronicsBoxProperties(ELECTRONICS_BOX::BOX_VERSION_1, m_electronicsBoxProperties);
}
...
void CSerialControllerWithTx::SendCommand(const char* commandLine)
{
    CString command;
...
{
    unsigned long maxchunk = m_electronicsBoxProperties.maxChunkSize;
    ...
    if (diskName == 'A' && m_electronicsBoxProperties.includeDiskNameInDataDownload)
    {
...
{
    if (!m_electronicsBoxProperties.needsToSwitchModes)
    {
        return;
    }
...
bool CSerialControllerWithTx::SwitchToDisk(char diskName)
{
    ...
    if (diskName == 'A')
    {
        nChar = sprintf(message, "%s\r\n", m_electronicsBoxProperties.command_EnterConfigurationFileDirectory.c_str());
    }
    else
    {
        nChar = sprintf(message, "%s\r\n", m_electronicsBoxProperties.command_EnterDataDirectory.c_str());
    }
...
#define SMALLBUFFER 2048
    ...
    if (!m_electronicsBoxProperties.needsToSwitchModes)
    {
        // never necessary to switch disk for the Axis box
        return SUCCESS;
    }
...
int CSerialControllerWithTx::OrganizeFileList(CString folderName, char diskName)
{
    ...
    char command[128];
    if (m_electronicsBoxProperties.includeDiskNameInDataDownload)
    {
...
void CSerialControllerWithTx::EnterFolder(const CString &folderName)
{
    ...
    if (folderName.GetLength() > m_electronicsBoxProperties.maxPathLength)
    {
        return;
    }
...
#define SEND_SIZE 3584 //512*7
    ...
    if (m_electronicsBoxProperties.uploadToTemporaryFile)
    {
        sprintf(nametoset, m_electronicsBoxProperties.temporaryUploadFileName.c_str());
    }
    else
    {
    ...
    if (m_electronicsBoxProperties.uploadToTemporaryFile)
    {
        ...
        if (diskName == 'A')
        {
            sprintf(nametoset, "mv %s %s/%s", m_electronicsBoxProperties.temporaryUploadFileName.c_str(), m_electronicsBoxProperties.configurationFileLocation.c_str(), name);
            SendCommand(nametoset);
        }
        else
        {
            sprintf(nametoset, "mv %s %s/%s", m_electronicsBoxProperties.temporaryUploadFileName.c_str(), m_electronicsBoxProperties.pakFileLocation.c_str(), name);
            SendCommand(nametoset);
        }
    }
...
{//out of tx
    char cmd[32];
    sprintf(cmd, "%s %s", m_electronicsBoxProperties.command_RemoveDirectory.c_str(), (LPCSTR)folder);
    Sleep(1000);
    SendCommand(cmd);
...
bool CSerialControllerWithTx::GetFileListText(CString& textA, CString& textB)
{
    ...
    if (m_electronicsBoxProperties.numberOfDisks == 2)
    {
...
{
    const char* command = m_electronicsBoxProperties.command_EnterDataDirectory.c_str();
    SendCommand(command);
}
...
int CSerialControllerWithTx::PollElectronicsType() {
    ...
    if (CheckSerial(m_timeout)) {
        ...
        {
            SetupElectronicsBoxProperties(BOX_VERSION_1, this->m_electronicsBoxProperties);
        }
