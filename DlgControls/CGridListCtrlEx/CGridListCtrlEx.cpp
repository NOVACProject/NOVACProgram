...
//------------------------------------------------------------------------
template<class T>
class GridListCtrl::COleDropTargetWnd : public ::COleDropTarget
{
    T* m_pWnd;
    bool m_DragSource;
...
public:
    explicit COleDataSourceWnd(COleDropTargetWnd<T>* pTarget)
        :m_pTarget(pTarget)
    {
        if (m_pTarget != NULL)
...
template<class T>
class GridListCtrl::COleDropSourceWnd : public ::COleDropSource
{
    T* m_pTarget;

public:
    explicit COleDropSourceWnd(T* pTarget)
        :m_pTarget(pTarget)
    {
    }
...
void CGridListCtrlEx::LoadState(CViewConfigSection& config)
{
    ...
    for (int i = nVisibleCols - 1; i >= 0; --i)
    {
        ...
        const int nColData = config.GetIntSetting(colSetting);
        for (int nCol = 0; nCol < nColCount; ++nCol)
        {
...
int CGridListCtrlEx::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    const int rc = CListCtrl::OnCreate(lpCreateStruct);
    if (rc == 0)
        OnCreateStyle();
...
int CGridListCtrlEx::InsertColumnTrait(int nCol, const CString& strColumnHeading, int nFormat, int nWidth, int nSubItem, CGridColumnTrait* pTrait)
{
    ...
    const int index = InsertColumn(nCol, static_cast<LPCTSTR>(strColumnHeading), nFormat, nWidth, nSubItem);
    if (index != -1)
    {
        VERIFY(index == nCol);
...
BOOL CGridListCtrlEx::GetCellRect(int nRow, int nCol, int nCode, CRect& rect)
{
    ...
    if (nCode == LVIR_ICON)
    {
        ...
        const int nImage = GetCellImage(nRow, nCol);
...
void CGridListCtrlEx::SetFocusCell(int nCol, bool bRedraw)
{
    ...
    if (bRedraw)
    {
        const int nFocusRow = GetFocusRow();
        if (nFocusRow >= 0)
        {
            CRect itemRect;
...
void CGridListCtrlEx::MoveFocusCell(bool bMoveRight)
{
    ...
    else
    {
        ...
        for (int i = 0; i < nColCount; ++i)
        {
            const int nCol = GetHeaderCtrl()->OrderToIndex(i);
    ...
    if (pNMW->item.mask & LVIF_IMAGE)
    {
        ...
        if (nCol == 0)
        {
            if (GetStyle() & LVS_OWNERDATA && GetExtendedStyle() & LVS_EX_CHECKBOXES)
            {
                ...
                const bool bChecked = OnOwnerDataDisplayCheckbox(nRow);
...
#endif
{
    ...
    pTI->rect = static_cast<RECT>(rcClient);
    if (headerRect.PtInRect(pt))
    {
        ...
        pTI->rect = static_cast<RECT>(headerRect);
    ...
    if (nRow != -1 && nCol != -1)
    {
        ...
        if (OnEditComplete(nRow, nCol, m_pEditor, pDispInfo))
        {
            *pResult = TRUE;	// Accept edit
            ...
            const bool txtEdit = pDispInfo->item.mask & LVIF_TEXT && pDispInfo->item.pszText != NULL;
            if (txtEdit)
            {
            ...
            const bool imgEdit = pDispInfo->item.mask & LVIF_IMAGE && pDispInfo->item.iImage != I_IMAGECALLBACK;
            if (imgEdit)
            {
                // Handle situation where data is stored inside the CListCtrl
                if (!(GetStyle() & LVS_OWNERDATA))
...
void CGridListCtrlEx::OnContextMenuKeyboard(CWnd* pWnd, CPoint point)
{
    ...
    else
    {
        ...
        const int cellHeightCenter = cellRect.Height() / 2;
        const int cellWidthCenter = cellRect.Width() / 2;
...
namespace {
    ...
    {
        PARAMSORT& ps = *reinterpret_cast<PARAMSORT*>(lParamSort);
...
DROPEFFECT CGridListCtrlEx::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
    ...
    if (GetItemCount() > 0)
    {
        ...
        const int scrollPos = GetScrollPos(SB_VERT);
...
namespace {
    ...
    {
        PARAMMOVESORT& ps = *reinterpret_cast<PARAMMOVESORT*>(lParamSort);

        // If part of selection, then it must be moved
        const bool selected1 = (ListView_GetItemState(ps.m_hWnd, lParam1, LVIS_SELECTED) & LVIS_SELECTED) == LVIS_SELECTED;
        const bool selected2 = (ListView_GetItemState(ps.m_hWnd, lParam2, LVIS_SELECTED) & LVIS_SELECTED) == LVIS_SELECTED;
...
bool CGridListCtrlEx::MoveSelectedRows(int nDropRow)
{
    ...
    const int nFirstSelectedRow = GetNextSelectedItem(pos);
